...
CATEGORIES = [
    ("работа/заработок", 3, [
        ...
        r"свободн\w*\s+врем", r"без\s+опыта", r"\bвакансии?\b",
        r"(требу\w*|ищем|нужн\w*)\s+(\d+\s+)?(человек|люди|людей|сотрудник|работник|помощни|девушк|парн|курьер"
        r"|грузчик|уборщиц|менеджер|оператор|исполнител)",
        r"уборк\w*\s+(после|квартир|офис|помещен)", r"график\s+свободн",
        r"(оплата|выплат\w*)\s+(ежедневн|каждый\s+день|сразу|в\s+конце\s+дня|на\s+карту)",
    ]),
    ("обещание денег", 3, [
        # «По 6000 день», «1500 за смену», «15к в день» — без валюты и предлога тоже
        r"\b\d{3,}[\d\s]*\s*(\$|₽|руб\w*|р\.?|тыс\w*|k|к|usdt?|евро|€)?\s*(в|за|/)?\s*(день|сутки|смену|час)\b",
        r"\b\d+\s*(тыс\w*|k|к)\s*(в|за|/)?\s*(день|сутки|смену|час|неделю|месяц)\b",
